...
func Distribute(ctx context.Context, command string, nodes ...infra.Node) {
	...
	Expect(infra.Distribute(ctx, command, nodes...)).To(Succeed())
}
...
// after the tests
func CoreDump(ctx context.Context) {
	if Cluster == nil {
	...
	if installerNode != nil {
		...
		err = infra.ScpText(ctx, installerNode, Cluster.Provisioner().InstallerLogPath(), installerLog)
		if err != nil {
	...
	for _, node := range Cluster.Provisioner().NodePool().Nodes() {
		...
		Expect(err).NotTo(HaveOccurred())
		errCopy := infra.ScpText(ctx, node, defaults.AgentLogPath, agentLog)
		agentLog.Close()
		if errCopy != nil {
...
func fetchReportLogs(ctx context.Context) error {
	reportCmd := fmt.Sprintf("gravity report --file %v", defaults.ReportPath)
	err := infra.Run(ctx, installerNode, reportCmd, os.Stderr)
	if err != nil {
	...
	err = infra.ScpText(ctx, installerNode, defaults.ReportPath, reportFile)
	if err != nil {
...
func RunAgentCommand(ctx context.Context, command string, nodes ...infra.Node) {
	command, err := infra.ConfigureAgentCommandRunDetached(command)
	Expect(err).NotTo(HaveOccurred())
	Distribute(ctx, command, nodes...)
}
